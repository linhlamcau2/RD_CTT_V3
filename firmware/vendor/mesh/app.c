...
//RD_EDIT: LIB_APP.C
#include "../RD_Add_RoomSceen/RD_Add_RoomSceen.h" //RD_EDIT: K9B
...
#if(1)
		mesh_cmd_bear_unseg_t *p_bear = GET_BEAR_FROM_ADV_PAYLOAD(pa->data);
...
if((KP9_DataRec->type == 0xff)&&(KP9_DataRec->lengt == 0x0e)){
		...
		/*---------check Scene-----------*/
		RD_K9B_ScanPress2HC(MacK9B_Buff, key_buff, signature_last);

	}
}
#endif // K9B_REMOTE_SUPPORT_ON

#if(0)// thanh
			mesh_cmd_bear_unseg_t *p_bear = GET_BEAR_FROM_ADV_PAYLOAD(pa->data);
			adv_report_extend_t *p_extend = get_adv_report_extend(&p_bear->len);
			RecDataK9B((switchK9B_proxy_t*)(pa->data), (u8 *)(pa->mac), p_extend->rssi);
#endif
...
//	test_simu_io_user_define_proc();
	...
	proc_ui();
	proc_led();
	if((flag_training_test2 != 1) && (step > 5)){	//RD_EDIT: khoa factory reset
		factory_reset_cnt_check();
	}
	#endif
	#if DU_LPN_EN
	...
	#else
//	mesh_loop_process();
	if((flag_training_test2 != 1) && (step > 5)){		//RD_EDIT: khoa mesh beacon
		mesh_loop_process();
	}
	else{
		system_time_run();
