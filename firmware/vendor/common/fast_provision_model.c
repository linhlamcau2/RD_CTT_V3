...
#define FAST_PROVISION_TIMEOUT 		3*60*1000*1000
...
void mesh_revert_network()
{
	...
	if((!fast_prov.not_need_prov)&&(mesh_fast_prov_sts_get() == FAST_PROV_COMPLETE)){
		...
		//rf_link_light_event_callback(LGT_CMD_SET_MESH_INFO);	//RD_EDIT: 0.1.THANH FAST_PROVISION_ENABLE rf_link_light_event_callback
	}
	else
	#endif
	{
