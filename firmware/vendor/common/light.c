...
const u32 GPIO_LED_INDEX = (GPIO_LED == PWM_R) ? 0 : ((GPIO_LED == PWM_G) ? 1 : ((GPIO_LED == PWM_B) ? 2 : ((GPIO_LED == PWM_W) ? 3 : ((GPIO_LED == PWM_Y) ? 4 :0))));
...
#if  (NAME == RLT03_12W_12026 || NAME == TRL08_10W_12025 || NAME == TRL08_20W_12024 || NAME == RLT01_10W_12023 || NAME == RLT02_20W_12022 || NAME == RLT02_10W_12021 || NAME == RLT03_06W_12020)		//RD_EDIT: 0.1.THANH GH_XG_RXG
	const u16 rgb_lumen_map[101] = {
		0,983 ,2*655,3*655,4*655,5*655,6*655,7*655,8*655,9*655,10*655,11*655,12*655,13*655,
		14*655,15*655,16*655,17*655,18*655,19*655,20*655,21*655,22*655,23*655,24*655,25*655,26*655,
		27*655,28*655,29*655,30*655,31*655,32*655,33*655,34*655,35*655,36*655,37*655,38*655,39*655,
		40*655,41*655,42*655,43*655,44*655,45*655,46*655,47*655,48*655,49*655,32768 ,51*655,52*655,
		53*655,54*655,55*655,56*655,57*655,58*655,59*655,60*655,61*655,62*655,63*655,64*655,65*655,
		66*655,67*655,68*655,69*655,70*655,71*655,72*655,73*655,74*655,75*655,76*655,77*655,78*655,
		79*655,80*655,81*655,82*655,83*655,84*655,85*655,86*655,87*655,88*655,89*655,90*655,91*655,
		92*655,93*655,94*655,95*655,96*655,97*655,98*655,99*655,65535,									// must less or equal than (255*256)
	};
#else
	const u16 rgb_lumen_map[101] = {
...
void light_res_sw_load()
{
	foreach_arr(i,light_res_sw){
		foreach_arr(k,light_res_sw[i].trans){
			...
			#else
			if((ONPOWER_UP_STORE == ONPOWER_UP_VAL(i)) || ((ST_TRANS_LIGHTNESS == k) && (analog_read(DEEP_ANA_REG0)&(FLD_OTA_REBOOT_FLAG|FLD_LOW_BATT_FLG)))){
			#endif
    ...
    foreach(i, LIGHT_CNT){
        ...
        #endif
        {
            ...
            if(onoff_present && CB_USER_LIGHT_INIT_ON_CONDITION()){
				#if !MI_SWITCH_LPN_EN
				light_transition_onoff_manual(G_ON, (analog_read(DEEP_ANA_REG0)&(FLD_OTA_REBOOT_FLAG|FLD_LOW_BATT_LOOP_FLG))?0:edch_is_exist()?g_def_trans_time_val(i):0, i);
				#endif
			}
        }
    }
...
_USER_CAN_REDEFINE_ void light_dim_refresh(int idx) // idx: index of LIGHT_CNT.
{
...
#if (SELECT_DIM == RD_DIM_0)		//RD_EDIT: cs cac loai den
    vrs_lum = lum_100;

#elif (SELECT_DIM == RD_DIM_10)
    vrs_lum = 10 + lum_100*0.9;		//10%

#elif (SELECT_DIM == RD_DIM_20)
    vrs_lum = 20 + lum_100*0.8;		//20%

#elif (SELECT_DIM == RD_DIM_25)
    vrs_lum = 25 + lum_100*0.75;	//25%
#endif
    ...
    light_dim_set_hw(idx, 2, get_pwm_cmp(0xff, (100-mi_ct)*lum_100/100));
            #else
    if(ct_flag){				//RD_EDIT: control dim/cct
		#if(NAME == LED_WIRE_CT)
		   light_dim_set_hw(idx, 0, get_pwm_cmp(0xff, (100- ct_100)*lum_100/100 )); //lum_100));			//RD_EDIT LED day
		   light_dim_set_hw(idx, 1, get_pwm_cmp(0xff, ct_100*lum_100/100 ));

		#elif (NAME == RLT03_12W_12026 || NAME == TRL08_10W_12025 || NAME == TRL08_20W_12024 || NAME == RLT01_10W_12023 || NAME == RLT02_20W_12022 || NAME == RLT02_10W_12021 || NAME == RLT03_06W_12020)	//RD_EDIT: 0.1.THANH GH_XG_RXG
		   if(lum_100 == 0){
			   light_dim_set_hw(idx, 0, get_pwm_cmp(0xff,(100- ct_100)*lum_100/100));
			   light_dim_set_hw(idx, 1, get_pwm_cmp(0xff,ct_100*lum_100/100));
		   }else{
	           light_dim_set_hw(idx, 0, get_pwm_cmp(0xff,
	        		   (((100- ct_100)*lum_100)<100)?1:(100- ct_100)*lum_100/100 )); //lum_100));			//RD_EDIT LED day
	           light_dim_set_hw(idx, 1, get_pwm_cmp(0xff,
	        		   ((ct_100*lum_100)<100)?1:ct_100*lum_100/100 ));
		   }
		#else
		   if(lum_100 == 0){
		...
		if( (u32)(clock_time() - led_tick) >= (led_is_on ? led_ton : led_toff) ){
			...
			if( led_off || led_on  ){
                if (led_sel & BIT(0))
                {
                    ...
                    #else
					#if(NAME == LED_WIRE_CT || NAME == RLT03_12W_12026 || NAME == TRL08_10W_12025 || NAME == TRL08_20W_12024 || NAME == RLT01_10W_12023 || NAME == RLT02_20W_12022 || NAME == RLT02_10W_12021 || NAME == RLT03_06W_12020)
						light_dim_set_hw(GPIO_LED_INDEX, 1, LED_INDICATE_VAL * led_on);
						light_dim_set_hw(GPIO_LED_INDEX, 0, 0);		//RD_EDIT: nhay bao
...
_USER_CAN_REDEFINE_ void rf_link_light_event_callback (u8 status)
{
    ...
    else if(status == LGT_CMD_SET_SENCE ){			//RD_EDIT: nhay set scene
...
void RD_light_ev_with_sleep(u32 count, u32 half_cycle_us)	//RD_EDIT: Ham nhay RD
{
	for(u32 i=0; i< count;i++){
#if(MODULE_WATCHDOG_ENABLE)
        wd_clear();
