...
#include "../../drivers/8258/uart.h"	//RD_EDIT:lib
...
#include "vendor/RD_Add_RoomSceen/RD_Add_RoomSceen.h"
...
void increase_reset_cnt ()
{
	...
	if(reset_cnt == 12){
	    ...
	    kick_out(0);		//RD_EDIT: 0.THANH : SPM: RESET CUNG: kick_out(0)
	}
...
int factory_reset(){
	...
	CB_USER_FACTORY_RESET_ADDITIONAL();
    flash_erase_sector(FLASH_ADR_RESET_CNT);    // at last should be better, when power off during factory reset erase.
    flash_erase_sector(RD_SECURE_FLASH_AREA);		//RD_EDIT: SEC : FLASH : delete flash security
    deleteFlashK9B();
...
void kick_out(int led_en){
    ...
    #if !WIN32
    if(led_en){
    	RD_light_ev_with_sleep(3, 500*1000);	//RD_EDIT: kick out app
	    sleep_ms(500);
	    wd_clear();
	    sleep_ms(500);
	    wd_clear();
